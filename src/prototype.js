...
(function () {
    ...
    try {
      try {
        ...
        history.pushState(
          null,
          null,
          './?manifest=true&href=' + manifest_url + '&document=' + iframe.src
        );
      }
    } catch (err) {
  ...
  next.addEventListener('click', function (event) {
    if (next.hasAttribute('href')) {
      ...
      iframe.style.height =
        window.innerHeight - navigation.scrollHeight - 5 + 'px';
    }
  ...
  function cacheManifest(url) {
    ...
    return Promise.all([
      cacheSpine(manifestJSON, url),
      cacheResources(manifestJSON, url),
    ]);
  }
  ...
  function cacheResources(manifestJSON, url) {
    return manifestJSON
      .then(function (manifest) {
        return manifest.resources.map(function (el) {
          return el.href;
        });
      })
      .then(function (data) {
  ...
  function initializeNavigation(url, document_url) {
    return getManifest(url)
      .then(function (json) {
        ...
        if (json.resources) {
          var all_resources = spn.concat(json.resources);
        } else {
          var all_resources = spn;
        }
    ...
    return getManifest(url)
      ...
      .then(function (spine) {
        ...
        if (current_index >= 0) {
          if (current_index > 0) {
            console.log(
              'Previous document is: ' + spine[current_index - 1].href
            );
